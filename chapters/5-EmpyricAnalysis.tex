...
Per la ricerca degli iperparametri è stata usata una tecnica di \textit{CrossValidation} disponibile nativamente in Spark ha identificato come configurazione ottimale~\ref{lst:best_params}, tra quelle fornite in input, $\texttt{layers} = [13, 64, 2]$, con $\texttt{maxIter} = 100$, $\texttt{stepSize} = 0.05$ e $\texttt{blockSize} = 128$, raggiungendo una baseline F1 di $0.9823$ sul test set pulito.
