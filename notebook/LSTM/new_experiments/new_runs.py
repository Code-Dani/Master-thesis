...
train_lstm_new.py
=================
FASE 2 — Training LSTM sui nuovi parquet generati da generate_noisy_datasets_v2.py.

Nuovi gruppi:
  1. duplicated  — "target1"       → parquet: duplicated_target1_{pct}pct/
  2. missing     — "TP3_Reservoirs" → parquet: missing_TP3_Reservoirs_{pct}pct/
  3. noise       — "TP3_Reservoirs" → parquet: noise_TP3_Reservoirs_{pct}pct/
  4. outliers    — "TP3_Reservoirs" → parquet: outliers_TP3_Reservoirs_{pct}pct/

I risultati vengono aggiunti a lstm_results_new.jsonl (resume automatico).

Esecuzione:
    python train_lstm_new.py
"""
...
TEST_DIR     = r"D:\Users\satri\Pictures\github\Deep-Learning-Robustness-Study\notebook\Dataset\NEW\noisy"
RESULTS_PATH = r"D:\Users\satri\Pictures\github\Deep-Learning-Robustness-Study\notebook\LSTM\new_experiments\lstm_results_new.jsonl"
...
# =============================================================================
# IPERPARAMETRI (identici a train_lstm.py)
# =============================================================================
BEST_WINDOW_SIZE = 30
BEST_HIDDEN_SIZE = 64
BEST_NUM_LAYERS  = 2
...
COMBINED_LABEL  = "TP3_Reservoirs"

# =============================================================================
# LSTM (identico a train_lstm.py)
# =============================================================================
...
def train_and_eval(
    ...
    for epoch in range(BEST_EPOCHS):
        ...
        print(f"      epoch {epoch+1:>2}/{BEST_EPOCHS}  "
              f"loss={avg_loss:.4f}  "
              f"({epoch_time:.1f}s/epoch  ETA: {eta:.0f}s)", flush=True)
...
def pq_path(noise_type: str, feature_label: str, percentage: float) -> str:
    """Ricostruisce il path del parquet dal naming di generate_noisy_datasets_v2."""
    if percentage == 0.0:
        return os.path.join(NOISY_DIR, "baseline")
    pct_str = str(int(round(percentage * 100)))
    if noise_type == "duplicated" and feature_label == "target1":
        return os.path.join(NOISY_DIR, f"duplicated_target1_{pct_str}pct")
...
def build_run_list() -> list[dict]:
    ...
    # Duplicated con target=1
    for pct in NOISE_LEVELS:
        combos.append({
            "noise_type":   "duplicated",
            "feature":      "target1",
            "percentage":   pct,
            "parquet_path": pq_path("duplicated", "target1", pct),
        })

    # Missing / Noise / Outliers su TP3+Reservoirs
    for noise_type in ["missing", "noise", "outliers"]:
        for pct in NOISE_LEVELS:
            combos.append({
                "noise_type":   noise_type,
                "feature":      COMBINED_LABEL,
                "percentage":   pct,
                "parquet_path": pq_path(noise_type, COMBINED_LABEL, pct),
            })
...
# =============================================================================
def main():
    test_path = os.path.join(TEST_DIR, "test_set")  
    ...
    print(f"\n{'='*65}")
    print(f"TRAINING LSTM NEW — {total_expected} run totali | {len(todo_runs)} rimanenti")
    print(f"Gruppi: duplicated/target1 + missing/noise/outliers su {COMBINED_LABEL}")
    print(f"Stima: {len(todo_runs) * 105 / 3600:.1f} ore (~105s/run)")
    print(f"{'='*65}\n")
        ...
        for i, run in enumerate(todo_runs):
            ...
            try:
                ...
                print(f"  ✅ F1={f1:.4f}  AUC={auc:.4f}  "
                      f"({elapsed:.1f}s)  |  ETA: {eta_h:.1f}h ({remaining} rimanenti)",
                      flush=True)
